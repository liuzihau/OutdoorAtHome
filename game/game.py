...
#定義環境
class Circle():
    def __init__(self,start=(55,35),length=(690,410),max_r = 60 ,prob=0.95,position='left_hand'):
...
def game_start(game_type):
    if game_type == "game1":
        lu = Circle(start=(55,35),length=(345,205),position='left_hand')
        lb = Circle(start=(155,200),length=(245,100),position='left_knee')
        ru = Circle(start=(400,35),length=(345,205),position='right_hand')
        rb = Circle(start=(400,240),length=(245,100),position='right_knee')
        return [lu,lb,ru,rb]
...
def game_plot(game_type,frame,env_coordinate):
    if game_type == "game1":
        color={'left_hand' :(80,80,180),
               ...
               'left_knee' :(80,180,80),
               'right_knee':(120,120,120)}
        for env in env_coordinate:
            if not env[0]:
                cv2.circle(frame,(env[1],env[2]),int(env[3]),color[env[4]],4)
        return frame
